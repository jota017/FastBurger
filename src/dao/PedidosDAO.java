...
public void ExcluirSanduiche(Pedidos pedi) {
    
    try {
    PreparedStatement stmt = connection.prepareStatement("DELETE FROM pedidosfb WHERE idpedi=?");
    stmt.setInt(1,pedi.getIdpedi());
    stmt.execute();
    JOptionPane.showMessageDialog(null, "Pedido excluido com sucesso!");
    } catch (SQLException ex) {
        JOptionPane.showMessageDialog(null, "Erro ao excluir o sanduiche!" + ex);
